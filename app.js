// =============================================================================
// ADVISION AI AGENT - FRONTEND LOGIC & API ENGINE
// =============================================================================
...
// SYSTEM PROMPT FOR ADVISION MASTER AGENT
...
function renderResults(rawText) {
  ...
  if (isPass) {
    verdictCard.className = "rounded-2xl p-6 border shadow-md flex flex-col sm:flex-row items-center justify-between gap-4 bg-emerald-500/10 border-emerald-300";
    verdictIcon.className = "w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shadow-md bg-emerald-500 text-white";
    verdictIcon.innerHTML = `<i class="fa-solid fa-circle-check"></i>`;
    verdictBadge.className = "inline-block text-xs font-extrabold px-3 py-1 rounded-full uppercase tracking-wider mb-1 bg-emerald-100 text-emerald-800 border border-emerald-300";
    verdictBadge.textContent = "KẾT LUẬN CHÍNH THỨC";
    verdictTitle.textContent = "✅ ĐẠT TIÊU CHUẨN QUẢNG CÁO";
    verdictScore.className = "text-2xl font-black text-emerald-700";
  } else {
    verdictCard.className = "rounded-2xl p-6 border shadow-md flex flex-col sm:flex-row items-center justify-between gap-4 bg-amber-500/10 border-amber-300";
    verdictIcon.className = "w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shadow-md bg-amber-500 text-white";
    verdictIcon.innerHTML = `<i class="fa-solid fa-triangle-exclamation"></i>`;
    verdictBadge.className = "inline-block text-xs font-extrabold px-3 py-1 rounded-full uppercase tracking-wider mb-1 bg-amber-100 text-amber-800 border border-amber-300";
    verdictBadge.textContent = "KẾT LUẬN CHÍNH THỨC";
    verdictTitle.textContent = "⚠️ CHƯA ĐẠT TIÊU CHUẨN (CẦN TỐI ƯU CHUYỂN ĐỔI)";
    verdictScore.className = "text-2xl font-black text-amber-700";
  }
...
function formatMarkdown(str) {
  if (!str) return "";
  let html = str
    .replace(/^### (.*$)/gim, '<h4 class="font-bold text-slate-900 mt-2 mb-1">$1</h4>')
    .replace(/^## (.*$)/gim, '<h3 class="font-bold text-slate-900 mt-3 mb-1 text-base">$1</h3>')
...
btnCopyReport.addEventListener('click', () => {
  ...
  alert("📋 Đã sao chép toàn bộ báo cáo phân tích!");
});
...
btnDownloadReport.addEventListener('click', () => {
  ...
  a.download = `Bao_Cao_Phan_Tich_AdVision_${Date.now()}.txt`;
  a.click();
});
...
btnSendReply.addEventListener('click', () => {
  ...
  const userMsgHtml = `<div class="mt-3 p-3 bg-indigo-100 rounded-xl text-indigo-900 font-semibold text-xs border border-indigo-200"><strong>Bạn:</strong> ${reply}</div>`;
  const aiMsgHtml = `<div class="mt-2 p-3 bg-white rounded-xl text-slate-800 font-medium text-xs border border-slate-200"><strong>AdVision Master:</strong> Cảm ơn thông tin cực kỳ hữu ích của bạn! Với nhóm đối tượng khách hàng này, chúng ta nên nhấn mạnh vào độ an toàn dịu nhẹ và thêm một thẻ quà tặng dùng thử ở góc phải banner để tăng gấp đôi tỷ lệ nhấp chuột nhé!</div>`;
...
[KHỐI 2: PHÂN TÍCH THỊ GIÁC & BỐ CỤC]
- Chủ thể & Sản phẩm chính: Hình ảnh sản phẩm trong banner (${filename}) hiển thị nổi bật ở trung tâm, thuộc ngành hàng ${category}.
- Văn bản & Chữ viết: Mật độ chữ khoảng 25% diện tích banner. Tiêu đề khuyến mãi rõ ràng nhưng phông chữ phụ hơi nhỏ.
- Thông điệp quảng cáo: Quảng bá dòng sản phẩm ưu đãi phù hợp cho nền tảng ${platform}.
- Nút kêu gọi hành động (CTA): Nút bấm CTA chưa đạt độ tương phản tối ưu so với phông nền.
